...
class MapViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate, AddIceReportDelegate, IceReportMapDelegate {
	...
	fileprivate func startLoadingData() {
		var daysToSearch = UserDefaults.standard.integer(forKey: "daysToSearch")
		if (daysToSearch < 1) {
			daysToSearch = 14
			UserDefaults.standard.set(daysToSearch, forKey: "daysToSearch")
		}
		let date = Date()
		...
		comps.day = -1 * daysToSearch
		
		var calendar = Calendar.current
		calendar.timeZone = TimeZone(identifier: "UTC")!
		let twoWeeksAgo = calendar.date(byAdding: comps, to: Date())
		
		let dbDate = dateFormat.string(from: twoWeeksAgo!)
